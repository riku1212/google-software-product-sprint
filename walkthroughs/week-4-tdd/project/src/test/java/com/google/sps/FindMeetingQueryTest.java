...
public final class FindMeetingQueryTest {
  ...
  @Test
  public void overlappingTime() {
    // Have an event for each person but one ends earlier. We should only see one option.
    //
    // Events  : |----A---|
    //           |--B--|         |--B--|
    // Day     : |---------------------|
    ...
    Collection<TimeRange> actual = query.query(events, request);
    Collection<TimeRange> expected = Arrays.asList(TimeRange.fromStartEnd(TIME_1000AM, TIME_1100AM, false));
